  ...
  document.getElementById('modal-content').innerHTML = `
    ...
    <div class="modal-body">
      ...
      <div class="modal-info">
        <!-- Badge de marketing en modal -->
        <span class="mktg-badge mktg-badge-modal">${badgeTexto}</span>
        <h2 class="modal-name">${p.name}</h2>

        <!-- Bloque de métricas sociales: visitas + rating -->
...
function init() {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', () => {
      ...
      debounceTimer = setTimeout(() => doSearch(true), 250);
    });
  ...
  if (searchBtn) searchBtn.addEventListener('click', () => doSearch(true));
