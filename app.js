...
async function generateProformaPDF() {
  ...
  const wantImages = chkImages && chkImages.checked;
  // Limite inteligente: sólo incluir imágenes si hay ≤ 3 productos
  const MAX_ITEMS_FOR_IMAGES = 3;
  const includeImages = wantImages && quotation.length <= MAX_ITEMS_FOR_IMAGES;

  /**
   * Convierte una URL de imagen al Data URL via <img> + canvas.
   * Devuelve null si falla (fallback silencioso).
   */
  function imageToDataURL(src) {
    return new Promise(resolve => {
      try {
        ...
        // Añadir timestamp sólo si la URL es relativa, para evitar caché stale en canvas
        img.src = src || '';
        if (!img.complete) {
          setTimeout(() => resolve(null), 8000); // timeout de seguridad
        }
      } catch { resolve(null); }
    });
  }

  // Obtener data-URLs de imágenes (en paralelo, con fallback)
  let imageDataMap = {}; // { productId: dataURL | null }
  if (includeImages) {
  ...
  // ── Bloque HTML de imágenes (sólo si aplica) ─────────────────────────────
  let imagesBlockHTML = '';
  if (includeImages) {
    const imgCols = quotation.map(item => {
      const dataURL = imageDataMap[item.id];
      if (!dataURL) return ''; // omitir imagen fallida silenciosamente
      const imgSize = quotation.length === 1 ? '160px' : '110px';
      return `
        <div style="text-align:center;flex:1;min-width:${imgSize};max-width:${imgSize};">
          <img src="${dataURL}" alt="${item.name}"
               style="width:${imgSize};height:${imgSize};object-fit:contain;border-radius:6px;border:1px solid #e2e8f0;" />
          <p style="font-size:0.65rem;color:#64748b;margin-top:4px;line-height:1.2;">${item.name}</p>
        </div>`;
    }).join('');

    if (imgCols.trim()) {
      imagesBlockHTML = `
        <div style="margin-top:14px;padding:10px 0;border-top:1px solid #e2e8f0;">
          <p style="font-size:0.68rem;color:#94a3b8;font-weight:600;letter-spacing:.04em;margin-bottom:8px;">IMÁGENES REFERENCIALES</p>
