 ...
 * Regla de Oro: La Proforma Principal (Página 1) NUNCA se distorsiona ni comprime.
 * Si se selecciona "Incluir imágenes", se crea automáticamente una Galería Anexa en Páginas A4 Adicionales (Página 2+).
 */
async function generateProformaPDF() {
  ...
  const chkImages = document.getElementById('chk-include-images');
  const includeImages = chkImages && chkImages.checked;

  /**
   * Convierte una URL de imagen al Data URL via <img> + canvas para incrustar limpiamente en PDF.
   */
  ...
  // Pre-cargar imágenes si se activó la opción
  let imageDataMap = {};
  if (includeImages) {
    const results = await Promise.all(
      quotation.map(item => imageToDataURL(item.image).then(d => ({ id: item.id, data: d })))
    );
    results.forEach(r => { imageDataMap[r.id] = r.data; });
  }
  ...
  // ── PÁGINA 1: PROFORMA PRINCIPAL (Limpieza y estructura garantizada) ──────
  let pdfHTML = `
  ...
  // ── PÁGINA 2+: GALERÍA DE IMÁGENES EN PÁGINAS ADICIONALES A4 ───────────────
  if (includeImages && quotation.length > 0) {
    const ITEMS_PER_GALLERY_PAGE = 4;
    const totalGalleryPages = Math.ceil(quotation.length / ITEMS_PER_GALLERY_PAGE);

    for (let pageIdx = 0; pageIdx < totalGalleryPages; pageIdx++) {
      const pageItems = quotation.slice(pageIdx * ITEMS_PER_GALLERY_PAGE, (pageIdx + 1) * ITEMS_PER_GALLERY_PAGE);

      const galleryCardsHTML = pageItems.map(item => {
        const dataURL = imageDataMap[item.id] || item.image;
        return `
          <div class="pdf-gallery-card">
            <div class="pdf-gallery-img-wrap">
              <img src="${dataURL}" alt="${item.name}" onerror="this.src='image/1000.jpg'" />
            </div>
            <div class="pdf-gallery-info">
              <p class="pdf-gallery-title">${item.name}</p>
              <p class="pdf-gallery-meta">${item.brand ? `Marca: ${item.brand} · ` : ''}Precio: S/ ${item.price.toFixed(2)}</p>
            </div>
          </div>
        `;
      }).join('');
      ...
      pdfHTML += `
        ...
        <div class="proforma-pdf-page proforma-pdf-gallery-page">
          <div class="pdf-gallery-header">
            <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #1e3a8a;padding-bottom:8px;margin-bottom:14px;">
              <div>
                <h2 style="font-size:1.1rem;font-weight:800;color:#1e3a8a;margin:0;">ANEXO: GALERÍA DE PRODUCTOS</h2>
                <p style="font-size:0.75rem;color:#475569;margin:2px 0 0;">Proforma N° ${codeDoc} · Cliente: ${clientName}</p>
              </div>
              <span style="font-size:0.75rem;font-weight:700;color:#1e3a8a;background:#f1f5f9;padding:4px 10px;border-radius:12px;border:1px solid #cbd5e1;">Página ${pageIdx + 2} de ${totalGalleryPages + 1}</span>
            </div>
          </div>
          <div class="pdf-gallery-grid">
            ${galleryCardsHTML}
          </div>
          <div class="pdf-gallery-footer" style="margin-top:auto;padding-top:10px;border-top:1px solid #cbd5e1;text-align:center;font-size:0.72rem;color:#64748b;">
            Elijo Tecnología · Imágenes referenciales sujetas a disponibilidad · www.elijotecnologia.com
          </div>
        </div>
      `;
    }
  ...
  const element = document.getElementById('proforma-pdf-container');
