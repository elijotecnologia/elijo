...
// ─── Introducción de marca con video ─────────────────────────────────────────
function initBrandIntro() {
  const overlay = document.getElementById('brand-intro-overlay');
  const video = document.getElementById('brand-intro-video');
  ...
  function endIntro() {
    ...
    sessionStorage.setItem('elijo_intro_played', 'true');
    
    overlay.classList.add('fade-out');
    document.body.classList.remove('intro-playing');
    document.body.classList.add('intro-fade-in');
    
    setTimeout(() => {
      overlay.remove();
      document.body.classList.remove('intro-fade-in');
      handleProductHash();
    }, 800);
  }
  ...
  if (playPromise !== undefined) {
    playPromise.catch(err => {
      console.warn('⚠️ Elijo Tech: Reproducción automática del video de marca bloqueada por el navegador:', err.message);
      endIntro();
    });
  }
...
async function generateProformaPDF() {
  ...
  try {
    ...
    const tableRowsHTML = quotation.map((item, idx) => {
      const p = PRODUCTS.find(x => String(x.id) === String(item.id)) || {};
      const specsList = Object.entries(p.specs || {}).map(([k, v]) => `${k}: ${v}`).join(' · ');
      const dataUrl = includeImages ? preparedImages[idx] : null;
      ...
      return `
        <tr>
          ...
          <td style="vertical-align:top;">
            ...
            ${item.id ? ` · <small style="color:#475569;">ID: ${item.id}</small>` : ''}
            ...
            ${includeImages && dataUrl ? `
              <div style="margin-top:8px;text-align:left;">
                <span style="font-size:9px;font-weight:700;color:#64748b;display:block;margin-bottom:2px;">IMAGEN REFERENCIAL:</span>
                <img src="${dataUrl}" class="pdf-table-image" alt="${item.name}" />
              </div>
            ` : ''}
          </td>
      ...
      if (itemsWithImages.length > 0) {
        const cardsHTML = itemsWithImages.map(({ item, dataUrl }) => `
          <div class="pdf-gallery-card">
            ...
            <div class="pdf-gallery-img-wrap" style="margin-top:8px;width:100%;height:140px;display:flex;align-items:center;justify-content:center;background:#f8fafc;border-radius:6px;padding:6px;box-sizing:border-box;">
              <img src="${dataUrl}" alt="${item.name}" style="max-width:100%;max-height:100%;object-fit:contain;" />
            </div>
          </div>
        `).join('');
        ...
        galleryHTML = `
          ...
          <div class="pdf-gallery-section" style="margin-top:20px;padding-top:14px;border-top:2px solid #1e3a8a;">
